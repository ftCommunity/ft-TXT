		...
		struct uvc_buffer *buf, const __u8 *data, int len)
{
	...
	static int lasttwolen=0;
	static int headeronlycnt=0;
#ifdef FT_HACK
	/* If we received two blocks with in total more than 600 bytes and then receive more than 5 header only frames, we likely had a buffer overrun */
	if( (buf != NULL) && (lasttwolen >= 6000) && (len == 12) && (buf->bytesused < buf->length) )
	{
	    headeronlycnt++;
	    if( headeronlycnt>=5 )
	    {
